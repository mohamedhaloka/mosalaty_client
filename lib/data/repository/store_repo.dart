...
class StoreRepo {
  ...
  Future<Response> getStoreList(int offset, String filterBy) async {
    return await apiClient
        .getData('${AppConstants.STORE_URI}/$filterBy?offset=$offset&limit=10');
  }

  Future<Response> getPopularStoreList(String type) async {
    return await apiClient
        .getData('${AppConstants.POPULAR_STORE_URI}?type=$type');
  }
  ...
  Future<Response> getStoreSearchItemList(
  ...
  Future<Response> getStoreReviewList(String storeID) async {
    return await apiClient
        .getData('${AppConstants.STORE_REVIEW_URI}?store_id=$storeID');
  }
}
