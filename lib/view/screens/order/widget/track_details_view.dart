      ...
      child: (!_takeAway && track.deliveryMan == null)
          ...
          : Column(children: [
              ...
              Row(children: [
                ...
                Expanded(
                  flex: 5,
                  child: (_takeAway && track.orderType != 'parcel')
                      ? Text(
                          track.store.address,
                          style: robotoRegular.copyWith(
                              fontSize: Dimensions.fontSizeSmall),
              ...
              _takeAway
                  ? InkWell(
                      onTap: () async {
                        String url =
                            'https://www.google.com/maps/dir/?api=1&destination=${track.store.latitude}'
              ...
              Row(children: [
                ClipOval(
                    child: CustomImage(
                  image:
                      '${_takeAway ? Get.find<SplashController>().configModel.baseUrls.storeImageUrl : Get.find<SplashController>().configModel.baseUrls.deliveryManImageUrl}/${_takeAway ? track.store.logo : track.deliveryMan.image}',
                  height: 35,
                  width: 35,
                  fit: BoxFit.cover,
                )),
                ...
                InkWell(
                  onTap: () async {
                    if (await canLaunchUrlString(
                        'tel:${_takeAway ? track.store.phone : track.deliveryMan.phone}')) {
                      launchUrlString(
                          'tel:${_takeAway ? track.store.phone : track.deliveryMan.phone}');
                    } else {
                      showCustomSnackBar(
                          '${'can_not_launch'.tr} ${_takeAway ? track.store.phone : track.deliveryMan.phone}');
                    }
                  },
                  child: Container(
